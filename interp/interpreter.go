	...
	for instIndex := 0; instIndex < len(bb.instructions); instIndex++ {
		...
		case llvm.Call:
			...
			case callFn.name == "runtime.trackPointer":
				...
				continue
			case callFn.name == "(reflect.Type).Elem" || strings.HasPrefix(callFn.name, "runtime.print") || callFn.name == "runtime._panic":
				// These functions should be run at compile time. Specifically:
